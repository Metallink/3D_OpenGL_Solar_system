...
void MyScene::draw()
{
  ...
  switch(_currentObject%_numberOfObjects) {
    ...
    glPushMatrix();
    glRotatef(_nbRotate,0,0,-1); //on l'oriente sur l'axe z
    glTranslatef(2,0,0);
    glRotatef(_nbRotatePlanete,0,0,-1); //on l'oriente sur l'axe z
    glScalef(0.3,0.3,0.3);
    glColor3f(242,245,249);
    glutSolidSphere(1, 20,10);
    //glTranslatef(_x + (radius * cos(twicePi / 100)), _y + (radius* sin(twicePi / 100)),0);
    glPopMatrix();
